...
class ModelWithPadding(eqx.Module):
    model: eqx.Module
    generalization_task: task.GeneralizationTask = eqx.field(static=True)
    ...
    def __call__(self, x: chex.Array, key: chex.PRNGKey) -> chex.Array:
        ...
        extra_zeros_x = jnp.zeros((input_length, final_input_size - input_size))
        x = jnp.concatenate([x, extra_zeros_x], axis=-1)

        computation_tape = jnp.full(
            (self.computation_steps_mult * input_length,),
        ...
        output_tokens = jnp.full(
            ...
            + OUTPUT_EMPTY_TOKEN
            - int(self.computation_steps_mult == 0),
        )
...
def make_model_with_empty_targets(
  ...
  """
  return ModelWithPadding(
    model=model,
    generalization_task=generalization_task,
    computation_steps_mult=computation_steps_mult,
    single_output=single_output,
  )

class ModelWithTargetsInput(eqx.Module):
  model: eqx.Module
  computation_steps_mult: int = eqx.field(static=True)
  ...
  def __call__(self, x: chex.Array, y: chex.Array, key: chex.PRNGKey) -> chex.Array:
    ...
    extra_zeros_x = jnp.zeros(
        (input_length, final_input_size - input_size)
    )
    x = jnp.concatenate([x, extra_zeros_x], axis=-1)
    extra_zeros_y = jnp.zeros(
        (output_length, final_input_size - output_size)
    )
    ...
    final_input = jnp.concatenate(
        [x, computation_tape, output_token, y], axis=0
    )
...
def make_model_with_targets_as_input(
    model: eqx.Module, computation_steps_mult: int = 0
) -> eqx.Module:
  ...
  return ModelWithTargetsInput(model, computation_steps_mult)

class SamplingToAutoregressiveModel(eqx.Module):
  model: eqx.Module
  single_output: bool = eqx.field(static=True)
  ...
  def __call__(self, x: chex.Array, y: chex.Array, key: chex.PRNGKey, sample: bool) -> chex.Array:
    ...
    Args:
      x: The input sequences of shape (t, i), where i is the input size.
      y: The target sequences of shape (t, o), where o is the output size.
      sample: Whether to evaluate the model using autoregressive decoding.
    """
    output_length = 1 if len(y.shape) == 1 else y.shape[0]
...
def add_sampling_to_autoregressive_model(
  ...
  return SamplingToAutoregressiveModel(model, single_output)
